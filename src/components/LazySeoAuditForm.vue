<template>
  <div ref="root" class="lazy-seo-audit-form">
    <Suspense v-if="shouldLoad">
      ...
      <template #fallback>
        <div class="flex items-center justify-center py-12" aria-busy="true">
          <div class="animate-spin h-8 w-8 border-4 border-accent-200 border-t-accent-600 rounded-full"></div>
...
const root = ref(null)
let observer
const AsyncSeoAuditForm = defineAsyncComponent(() => import('./SeoAuditForm.vue'))
const load = entries => {
  entries.forEach(e => {
    if (e.isIntersecting) {
