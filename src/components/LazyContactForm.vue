    ...
    <Suspense v-if="shouldLoad">
      ...
      <template #fallback>
        <div class="flex items-center justify-center py-16" aria-busy="true">
          <div class="animate-spin h-8 w-8 border-4 border-primary-200 border-t-primary-500 rounded-full"></div>
          <span class="sr-only">Loading contact form...</span>
        </div>
      </template>
    </Suspense>
    <noscript>
      <p class="text-sm text-neutral-600">Enable JavaScript to load the contact form.</p>
    </noscript>
  </div>
</template>

<script setup>
import { ref, onMounted, onBeforeUnmount, defineAsyncComponent, watchEffect } from 'vue'
...
const AsyncContactForm = defineAsyncComponent(() => import('./ContactForm.vue'))
...
onMounted(() => {
  ...
  if ('IntersectionObserver' in window) {
    observer = new IntersectionObserver(loadIfIntersecting, { rootMargin: '200px 0px' })
