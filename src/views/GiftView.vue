  ...
  <div class="container-site section-padding">
    <div class="max-w-5xl mx-auto">
      ...
      <p class="text-center text-lg text-neutral-600 mb-10">
        Enter your email to unlock our interactive 100+ point SEO checklist.
        We'll also send occasional high‑value SEO tips (you can unsubscribe
        anytime).
      </p>
        ...
        <div class="p-6 md:p-8">
          <div class="text-center mb-8">
            ...
            <p class="text-neutral-700">
              No spam. Just practical optimization value. Your progress saves
              locally.
            </p>
          </div>
          ...
          <form @submit.prevent="submitEmail" class="space-y-6" novalidate>
            ...
            <div class="flex items-start gap-2">
              ...
              <label for="terms" class="text-sm text-neutral-700">
                I agree to receive SEO resources from Ottawa Web Masters.
          ...
          >
            <p>
              We respect your privacy. Unsubscribe any time. We never sell your
              data.
            </p>
          </div>
        </div>
      ...
      <div
        v-else-if="isSubmitting && !checklistVisible"
      ...
      >
        ...
        <p class="text-neutral-700 mb-8">
          Preparing your interactive checklist...
        </p>
        <div
          class="w-full max-w-sm mx-auto bg-gray-200 rounded-full h-2.5 overflow-hidden"
        >
          <div
      ...
      <div v-else v-show="checklistVisible" class="mt-14 animate-fade-in">
        ...
        >
          ...
          <div class="flex gap-4">
            <button @click="resetAll" class="btn-outline btn-sm">
              Reset All
            </button>
        ...
        <div class="mb-6 flex flex-wrap gap-4 items-center">
          ...
          <div class="flex gap-2">
            <button
              @click="filter = 'all'"
              :class="filter === 'all' ? activeBtn : inactiveBtn"
            ...
            <button
              ...
              :class="filter === 'completed' ? activeBtn : inactiveBtn"
              class="px-3 py-1 rounded"
            >
              Completed
            </button>
            <button
              ...
              :class="filter === 'incomplete' ? activeBtn : inactiveBtn"
              class="px-3 py-1 rounded"
            >
              Incomplete
            </button>
            ...
            <div v-if="openSections.includes(sIdx)" class="p-4 bg-white">
              <div class="space-y-2">
                ...
                >
                  ...
                  />
                  <label
                    :for="`gift-item-${sIdx}-${iIdx}`"
                    class="flex-1 cursor-pointer"
                    :class="{
                      'line-through text-gray-400': checked[`${sIdx}-${iIdx}`],
                    }"
                    >{{ item }}</label
                  >
                </div>
        ...
        <div class="bg-primary-50 rounded-lg p-6 text-center mt-10">
          ...
          <p class="mb-6">
            We can turn this checklist into a prioritized strategy tailored to
            your site.
          </p>
...
const totalCount = computed(() =>
  checklistData.reduce((t, s) => t + s.items.length, 0)
);
  ...
  return checklistData
    .map(section => ({
      ...
      items: section.items.filter((item, idx) => {
        ...
        const matchesFilter =
          filter.value === 'all' ||
          (filter.value === 'completed' && checked.value[key]) ||
          (filter.value === 'incomplete' && !checked.value[key]);
...
const getCompleted = (section, idx) =>
  section.items.reduce(
    (c, _it, i) => c + (checked.value[`${idx}-${i}`] ? 1 : 0),
    0
  );
const toggleSection = idx => {
  openSections.value = openSections.value.includes(idx)
    ? openSections.value.filter(i => i !== idx)
    : [...openSections.value, idx];
};

function saveProgress() {
  localStorage.setItem(
    'gift_checklist_progress',
...
function resetAll() {
  if (confirm('Reset all progress?')) {
    checked.value = {};
    localStorage.removeItem('gift_checklist_progress');
  }
}
...
async function submitEmail() {
  ...
  try {
    const res = await fetch('/api/submit', {
      ...
      body: JSON.stringify({
        email: email.value,
        message: 'Gift SEO Checklist Access',
      }),
    });
    ...
    if (typeof window.trackEvent === 'function') {
      window.trackEvent('lead_capture', {
        form_type: 'gift_checklist_access',
        source: 'gift_page',
      });
    }
...
watch(
  checked,
  (val, oldVal) => {
    localStorage.setItem('gift_checklist_progress', JSON.stringify(val));
    // Track incremental completion events
    if (typeof window.trackEvent === 'function') {
      const oldCompleted = Object.values(oldVal || {}).filter(Boolean).length;
      const newCompleted = Object.values(val || {}).filter(Boolean).length;
      if (newCompleted > oldCompleted) {
        window.trackEvent('checklist_item_completed', {
          checklist: 'gift',
          total_completed: newCompleted,
          completion_percentage: Math.round(
            (newCompleted / totalCount.value) * 100
          ),
        });
      }
    }
...
onMounted(() => {
  if (localStorage.getItem('gift_access') === 'true') {
    hasAccess.value = true;
    checklistVisible.value = true;
  }
  ...
  if (typeof window.trackEvent === 'function') {
    window.trackEvent('page_view', { page: 'gift_checklist' });
  }
});
</script>
