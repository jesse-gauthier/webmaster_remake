<template>
  <div>
    <!-- Hero Section -->
    <section aria-labelledby="portfolio-hero-title" class="bg-primary-light py-16 md:py-24">
      <div class="container-site px-4 sm:px-6">
        <div class="max-w-3xl mx-auto text-center">
          <h1 id="portfolio-hero-title" class="text-3xl md:text-4xl lg:text-5xl font-bold text-primary mb-6">
            Our Portfolio
          </h1>
          <p class="text-lg text-neutral-text mb-8">
            Explore our collection of successful projects. Each one represents our commitment to quality,
            creativity, and delivering results that exceed client expectations.
          </p>
        </div>
      </div>
    </section>
      ...
      <div class="container-site px-4 sm:px-6">
        ...
        <div class="mb-12">
          <div class="flex flex-wrap justify-center gap-3 mb-8">
            <button @click="activeCategory = 'all'" class="px-5 py-2 rounded-full text-sm font-medium transition-colors"
              :class="activeCategory === 'all' ? 'bg-primary text-white' : 'bg-neutral-100 text-neutral-text hover:bg-neutral-200'"
              aria-pressed="activeCategory === 'all'">
              All Projects
            </button>
            <button v-for="category in uniqueCategories" :key="category" @click="activeCategory = category"
              class="px-5 py-2 rounded-full text-sm font-medium transition-colors capitalize"
              :class="activeCategory === category ? 'bg-primary text-white' : 'bg-neutral-100 text-neutral-text hover:bg-neutral-200'"
          ...
          <div v-for="project in filteredProjects" :key="project.id"
            ...
            <div class="relative overflow-hidden aspect-[4/3]">
              ...
              <div class="absolute top-4 right-4">
                <span class="inline-block bg-primary text-white text-xs px-3 py-1 rounded-full capitalize">
                  {{ project.category }}
                </span>
              </div>
            </div>
            ...
            <div class="p-6">
              ...
              <p class="text-neutral-text mb-4 line-clamp-3">{{ project.description }}</p>
        ...
        <div v-if="filteredProjects.length === 0" class="text-center py-12">
          <p class="text-lg text-neutral-text">No projects found in this category. Please try another filter.</p>
        </div>
      </div>
    </section>

    <!-- Call to Action -->
    <section aria-labelledby="portfolio-cta-title" class="bg-accent py-16 md:py-24">
      <div class="container-site px-4 sm:px-6">
        <div class="text-center max-w-3xl mx-auto">
          <h2 id="portfolio-cta-title" class="text-3xl md:text-4xl font-bold text-white mb-6">
...
// Portfolio items data - easy to add or remove projects
const portfolioItems = [
  {
    id: 'county-cooperage',
    title: 'The County Cooperage',
    category: 'web design',
    description: 'A ecomerce website for a wine barrel manufacturer featuring product listings, custom orders, and a blog with industry news and tips.',
...
const filteredProjects = computed(() => {
  if (activeCategory.value === 'all') {
    ...
    return portfolioItems.filter(project => project.category === activeCategory.value)
  }
})
</script>
